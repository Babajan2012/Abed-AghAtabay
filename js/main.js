...
(function () {
    ...
    const i18n = {
        fa: {
            ...
            "hero.roles": ["توسعه‌دهنده فرانت‌اند", "طراح رابط کاربری", "علاقه‌مند به تجربه کاربری"],
            "hero.desc": "به وب‌سایت شخصی من خوش آمدید. من عاشق طراحی تجربه‌های کاربری زیبا، مدرن و کارآمد هستم.",
            ...
            "about.p1": "من عابد هستم، توسعه‌دهنده فرانت‌اند با علاقه به ساخت وب‌سایت‌های مدرن و ریسپانسیو. همیشه به دنبال یادگیری تکنولوژی‌های جدید و بهبود مهارت‌های خود هستم.",
            "about.p2": "این قالب به گونه‌ای طراحی شده که پایه‌ای محکم برای یک سایت شخصی حرفه‌ای باشد. همه بخش‌ها کاملاً ریسپانسیو و با انیمیشن‌های نرم ساخته شده‌اند.",
            "about.name": "نام:",
            ...
            "footer.about": "توسعه‌دهنده فرانت‌اند با علاقه به طراحی وب‌سایت‌های مدرن، ریسپانسیو و کاربرپسند.",
        ...
        en: {
            ...
            "hero.desc": "Welcome to my personal website. I love crafting beautiful, modern and efficient user experiences.",
            "hero.btnPortfolio": "View Projects",
            "hero.btnContact": "Contact Me",
            "hero.scroll": "Scroll down",
            "about-head": "About Me",
            "about.title": "Who am I?",
            "about.p1": "I'm Abed, a frontend developer passionate about building modern and responsive websites. I'm always eager to learn new technologies and improve my skills.",
            "about.p2": "This template is designed as a solid base for a professional personal website. All sections are fully responsive and built with smooth animations.",
            "about.name": "Name:",
            ...
            "footer.about": "Frontend developer passionate about crafting modern, responsive and user-friendly websites.",
    ...
    const savedLang = localStorage.getItem("abed_lang") || "en";
    ...
    function startTyping(lang) {
        ...
        function tick() {
            ...
            ci += del ? -1 : 1;
            let spd = del ? 40 : 80;
            if (!del && ci === w.length) { spd = 1600; del = true; }
    ...
    function setLang(lang) {
        ...
        if (offcanvas) {
            ...
            offcanvas.classList.add(lang === "fa" ? "offcanvas-start" : "offcanvas-end");
        }
    ...
    function setTheme(theme) {
        ...
        if (icon) {
            icon.className = "bi " + (theme === "dark" ? "bi-sun-fill" : "bi-moon-stars") + " theme-icon";
    ...
    function setActive() {
        ...
        sections.forEach(s => { if (sp >= s.offsetTop) cur = s.id; });
        navLinks.forEach(l => l.classList.toggle("active", l.getAttribute("href") === "#" + cur));
    }

    /* ---------- Reveal ---------- */
    const ro = new IntersectionObserver(es => {
        es.forEach(e => { if (e.isIntersecting) { e.target.classList.add("in-view"); ro.unobserve(e.target); } });
    }, { threshold: 0.15 });
    document.querySelectorAll(".reveal").forEach(el => ro.observe(el));

    /* ---------- Skill bars ---------- */
    const so = new IntersectionObserver(es => {
        es.forEach(e => { if (e.isIntersecting) { e.target.classList.add("in-view"); so.unobserve(e.target); } });
    }, { threshold: 0.4 });
    ...
    const co = new IntersectionObserver(es => {
        es.forEach(e => {
            if (e.isIntersecting) {
                const el = e.target, tgt = +el.dataset.count, dur = 1400, st = performance.now();
                function step(now) {
                    const p = Math.min((now - st) / dur, 1);
                    el.textContent = Math.floor((1 - Math.pow(1 - p, 3)) * tgt);
                    if (p < 1) requestAnimationFrame(step); else el.textContent = tgt;
                }
    ...
    if (form) {
        form.addEventListener("submit", e => {
            e.preventDefault();
            const n = document.getElementById("name"), em = document.getElementById("email"),
                  s = document.getElementById("subject"), m = document.getElementById("message");
            if (!n.value.trim() || !em.value.trim() || !s.value.trim() || !m.value.trim()) {
            ...
            btn.disabled = true;
            btn.innerHTML = '<i class="bi bi-hourglass-split me-2"></i>' + i18n[currentLang]["form.sending"];
            setTimeout(() => {
    ...
    window.addEventListener("scroll", () => { setActive(); navbar.classList.toggle("scrolled", window.scrollY > 30); });
    document.getElementById("themeToggle").addEventListener("click", () => setTheme(currentTheme === "dark" ? "light" : "dark"));
    document.getElementById("langToggle").addEventListener("click", () => setLang(currentLang === "fa" ? "en" : "fa"));
